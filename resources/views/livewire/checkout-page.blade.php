<div class="max-w-6xl mx-auto px-4 py-10">
    ...
    $s2 = $current >= 2;
    $s3 = $current >= 3;
    // Progreso móvil: 1=>0%, 2=>50%, 3=>100%
    $progress = [1 => 0, 2 => 50, 3 => 100][$current] ?? 0;
    @endphp

    <div class="mt-6">
        {{-- Desktop / Tablet (≥ md) --}}
        <ol class="hidden md:flex items-center gap-4 text-sm">
            <li class="flex items-center gap-2">
                <div
                    class="w-7 h-7 rounded-full flex items-center justify-center {{ $s1 ? 'bg-[#19355C] text-white' : 'bg-gray-200 text-gray-600' }}">
                    1</div>
            ...
            <li class="flex items-center gap-2">
                <div
                    class="w-7 h-7 rounded-full flex items-center justify-center {{ $s2 ? 'bg-[#19355C] text-white' : 'bg-gray-200 text-gray-600' }}">
                    2</div>
            ...
            <li class="flex items-center gap-2">
                <div
                    class="w-7 h-7 rounded-full flex items-center justify-center {{ $s3 ? 'bg-[#19355C] text-white' : 'bg-gray-200 text-gray-600' }}">
                    3</div>
                <span class="font-medium {{ $s3 ? 'text-[#19355C]' : 'text-gray-500' }}">Pago</span>
            </li>
        </ol>

        {{-- Móvil (< md) --}} <div class="md:hidden">
            <div class="grid grid-cols-3 text-center text-[11px] sm:text-xs">
                <div class="flex flex-col items-center gap-1">
                    <div
                        class="w-7 h-7 rounded-full flex items-center justify-center {{ $s1 ? 'bg-[#19355C] text-white' : 'bg-gray-200 text-gray-600' }}">
                        1</div>
                    <span class="font-medium {{ $s1 ? 'text-[#19355C]' : 'text-gray-500' }}">Compra</span>
                </div>
                <div class="flex flex-col items-center gap-1">
                    <div
                        class="w-7 h-7 rounded-full flex items-center justify-center {{ $s2 ? 'bg-[#19355C] text-white' : 'bg-gray-200 text-gray-600' }}">
            ...
            {{-- Barra de progreso --}}
            <div class="mt-3 h-1.5 bg-gray-200 rounded">
                <div class="h-full bg-[#19355C] rounded transition-all" style="width: {{ $progress }}%"></div>
            </div>
    </div>
</div>
                ...
                <div class="space-y-3">
                    @for ($i = 0; $i < (int) $item['qty']; $i++) <div class="grid grid-cols-12 gap-3 items-end"
                        ...
                        <div class="col-span-12 sm:col-span-2">
                            <button type="button"
                                class="w-full px-3 py-2 text-xs rounded-md border hover:bg-white cursor-pointer"
                                wire:click="useBuyerFor('{{ $item['key'] }}', {{ $i }})" wire:loading.attr="disabled"
                                wire:target="useBuyerFor">
    ...
    <div class="p-5 rounded-xl border border-gray-200 bg-white space-y-6">
        ...
        <div class="space-y-3">
            <label class="flex items-center bg-gray-100 p-4 rounded-lg border cursor-pointer">
                <input type="radio" name="payment_method" value="webpayplus" class="mr-3" wire:model="payment_method">
        ...
        {{-- <label class="flex items-start gap-3">
            ...
            <span class="text-sm text-gray-700">
                Acepto los <a href="{{ url('/terminos') }}" target="_blank" class="text-[#19355C] underline">Términos y
                    Condiciones</a>.
            </span>
        </label> --}}
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            @if (($step ?? 1) < 3) <button type="button" class="px-4 py-2 rounded-lg
                ...
                @else
                {{-- Navegación de pasos (solo el bloque del botón final) --}}
                <form id="checkoutForm" wire:submit.prevent="confirmAndPay">
                    <button id="payBtn" type="submit" class="px-4 py-2 rounded-lg bg-[#19355C] text-white text-sm font-semibold hover:bg-[#47A8DF] transition
               disabled:opacity-50 disabled:cursor-not-allowed" wire:loading.attr="disabled"
                        wire:target="confirmAndPay" data-default="Confirmar y pagar" data-loading="Procesando…">
...
<aside class="lg:col-span-1">
    <div class="lg:sticky lg:top-6">
        <div class="p-5 rounded-xl border border-gray-200 bg-white">
            ...
            <a href="{{ route('cart.index') }}" class="block text-center text-sm text-[#19355C] hover:underline mt-3">
                Volver al carrito
            </a>
            @endif
        </div>
...
<script>
    (function () {
  // Capturamos TODOS los submit a nivel documento y filtramos por el form
  document.addEventListener('submit', function (e) {
    if (!e.target || e.target.id !== 'checkoutForm') return;

    const btn = e.target.querySelector('#payBtn');
    const label = btn?.querySelector('.btn-text');

    // Bloqueamos justo DESPUÉS de que el submit ya se disparó
    // para no cancelar la redirección de Livewire.
    setTimeout(() => {
      if (!btn) return;
      btn.disabled = true;
      btn.style.pointerEvents = 'none';
      if (label) label.textContent = btn.dataset.loading || 'Procesando…';
    }, 0);
  }, true); // <-- capturing = true, así funciona aunque Livewire reemplace el DOM

  // Si el servidor decide que NO se redirige (errores), re-habilitamos:
  document.addEventListener('livewire:load', () => {
    if (window.Livewire && typeof Livewire.on === 'function') {
      Livewire.on('unlockPayBtn', () => {
        const btn = document.getElementById('payBtn');
        const label = btn?.querySelector('.btn-text');
        if (!btn) return;
        btn.disabled = false;
        btn.style.pointerEvents = '';
        if (label) label.textContent = btn.dataset.default || 'Confirmar y pagar';
      });
    }
  });
})();
