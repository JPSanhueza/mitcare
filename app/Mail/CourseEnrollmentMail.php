...
class CourseEnrollmentMail extends Mailable
{
    ...
    public function build()
    {
        ...
        return $this
            ->subject('Inscripción confirmada: '. strip_tags_for_order_name($this->item->course_name))
            // Útil si quieres que respondan a soporte:
            // ->replyTo(config('mail.from.address'), config('mail.from.name'))
            ->markdown('mail.course-enrollment', [
                'previewText'  => '¡Listo! Confirmamos tu cupo en '. strip_tags_for_order_name($this->item->course_name) .'. Aquí te contamos los próximos pasos.',
                // 'courseUrl'    => $courseUrl,
